...
function getNextFriday(date) {
  let i = date.getUTCDate();
  const friday = date;
  do {
    i += 1;
    friday.setUTCDate(i);
  } while (friday.getUTCDay() !== 5);
  return friday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timeInDays = Math.floor(timeInMilliseconds / 86400000) + 1;
  return timeInDays;
}
...
function getCountWeekendsInMonth(month, year) {
  const previosMonth = new Date(year, month, 0);
  const numOfDays = previosMonth.getDate();
  let numOfWeekends = 0;
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil((timeInMili / 86400000 + startDay + 1) / 7);
}
